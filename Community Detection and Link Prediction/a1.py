...
def example_graph():
    """
    Create an example graph ,used for testing.
    Do not modify.
    """
    g = nx.Graph()
...
def bfs(graph, root, max_depth):
    ...
    node2parents = {root: []}
    max_parent_dist={root: 0}
    while (cdepth <= max_depth):
        for idx in range(idx, len(node_list), 1):
            cur_nd = node_list[idx]
            #print("new cuurrent node"+cur_nd)
            edges = list(nx.all_neighbors(graph, cur_nd))
            #print('Edges for ' + str(cur_nd) + ' : ' + str(edges))

            for ed in edges:
                if ed not in node2distances:
                    node2distances[ed] = cdepth;
                    node_list.append(ed)
                    node2num_paths[ed] = 1;
                    tmp_list = []
                    tmp_list.append(cur_nd)
                    if tmp_list is None:
                        print ('None data found');
                    node2parents[ed] = tmp_list
                    max_parent_dist[ed] = cdepth
                else:
                    parent_list = node2parents[ed]
                    if (ed not in node2parents[cur_nd]):
                       if (node2parents[ed] not in node2parents[cur_nd]):
                            if( cdepth <= max_parent_dist[ed]):
                                parent_list.append(cur_nd)
                                node2parents[ed] = sorted(parent_list)
                                node2num_paths[ed] = node2num_paths[ed] + 1;
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
  ...
  >>> node2distances, node2num_paths, node2parents = bfs(example_graph(), 'E', 5)
    ...
    subnodes =  set(node2parents.keys())
    merged = set(itertools.chain.from_iterable(node2parents.values()))
    leafs = sorted(list(subnodes - merged))
    nonleaf = list(set(merged | subnodes) - set(leafs))
    merged = leafs + nonleaf
    q = deque()
    edge_q=deque()
    edge_weight = {}
    visited =[root]
    ...
    for nd in merged:
        if(nd != root):
            node_weights[nd]=(1/len(node2parents[nd]))
        if(nd in leafs):
            q.append(nd)

    while(len(q) > 0):
        curr_nd = q.popleft()
        #print ("current node " +curr_nd)
        curr_parents = node2parents[curr_nd]
        #print("current node " + curr_nd +" Current Parents "+str(curr_parents))
        curr_nd_weight = node_weights[curr_nd]
        for par in curr_parents:
            if (par != root):
                #print ("current weight "+str(curr_nd_weight))
                #print ("Partent -"+str(par))
                #print("Before parent weight  "+str(node_weights[par]))
                node_weights[par] = node_weights[par] + node_weights[curr_nd]
                #print("After parent weight  " + str(node_weights[par]))
                if(par not in visited):
                    #print ("new added to Queue"+par)
                    q.append(par)
                    visited.append(par)
    ...
    while(len(q)>0):
        curr = q.popleft()
        for par in node2parents[curr]:
            edge = (curr,par) if curr < par else (par,curr)
            #edge_weight = node_weights[curr] if node_weights[curr] < node_weights[par] else node_weights[par]
            edge_weight = node_weights[curr]
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in graph.nodes():
        ...
        #print(type(result))
        for key in result:
             if(key not in node_betweeness):
                 node_betweeness[key]=0
             node_betweeness[key]+=result[key]

    for key in node_betweeness:
        node_betweeness[key]=node_betweeness[key]/2
    return node_betweeness
...
def partition_girvan_newman(graph, max_depth):
    ...
    start_time = time()
    newgraph = graph.copy()
    eb = approximate_betweenness(newgraph, max_depth)
    def find_best_edge(eb):
        maxvalue = 0
        alledges=[]
        maxvalue=max(eb.items(), key=operator.itemgetter(1))[1]
        #print (maxvalue)
        for key in eb.keys():
            if(eb[key]==maxvalue):
                alledges.append(key)
        return ((sorted(alledges))[0])

    start_time = time()
    components = [c for c in nx.connected_component_subgraphs(newgraph)]
    while len(components)<2 :
        edgetoremove=find_best_edge(eb)
        newgraph.remove_edge(*edgetoremove)
        del eb[edgetoremove]
        components = [c for c in nx.connected_component_subgraphs(newgraph)]
    return components
...
def get_subgraph(graph, min_degree):
    ...
    delete_node=[]
    subgraph=graph;
    for n in subgraph.__iter__():
        if(subgraph.degree(n)<min_degree):
            delete_node.append(n)
    subgraph.remove_nodes_from(delete_node)
    return  subgraph;
...
def volume(nodes, graph):
    ...
    for edges in graph.edges():
        edge1,edge2=edges
        if (edge1 in current) or (edge2 in current):
            count=count+1
    return count
...
def norm_cut(S, T, graph):
    ...
    norm_value=(cut1/vol_s)+(cut1/vol_t)
    return norm_value

def score_max_depths(graph, max_depths):
    """
    In order to assess the quality of the approximate partitioning method. we will run it with different values for max_depth
    ...
    for items in max_depths:
        ...
        components = sorted(components, key=lambda x: sorted(x.nodes())[0])
        norm=norm_cut(components[0].nodes(),components[1].nodes(),graph)
        value=(items,norm)
        scores.append(value)
    return scores
...
def make_training_graph(graph, test_node, n):
    """
    To make a training graph, we need to remove n edges from the graph. Remove the edges to the first n neighbors of
    ...
    edges=(sorted(test_graph.neighbors(test_node))[:n])
    for edge in edges:
        test_graph.remove_edge(test_node,edge)
    return test_graph
...
def jaccard(graph, node, k):
    ...
    temp2=list(sorted(temp, key=lambda x:x[0]))
    for items in temp2:
        item1,item2=items
        scores.append(((node,item1),item2))
...
def path_score(graph, root, k, beta):
    """
    Computes link prediction scoring function based on the shortest
    ...
    for n in graph.nodes():
        if (n != root and not graph.has_edge(root, n)):
            neighbors2 = set(graph.neighbors(n))
            path_length=node2distances.get(n)
            no_paths=node2num_paths.get(n)
            score=((beta ** (path_length))*no_paths)
            allscores.append(score)
            if (score not in allscores):
    ...
    for items in temp2:
        item1, item2 = items
        scores.append(((root, item1), item2))
    return sorted(scores, key=lambda x: x[1], reverse=True)[:k]
...
def evaluate(predicted_edges, graph):
    ...
    """
    in_graph=0
    length_predictededges=len(predicted_edges)
    for edges in predicted_edges:
        if  (graph.has_edge(*edges)):
            in_graph+=1
    return in_graph/length_predictededges
